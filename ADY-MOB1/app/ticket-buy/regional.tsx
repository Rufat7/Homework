...
const regionalLocations = ["Baku", "Ucar", "Agdas", "Gence", "Tovuz", "Agstafa"];
...
export default function Regional() {
  ...
  const Card = ({ text, selected, onPress }: { text: string; selected: boolean; onPress: () => void }) => (
    <TouchableOpacity
      onPress={onPress}
      style={{
        paddingVertical: 12,
  ...
  useEffect(() => {
    if (from && schedule[from]) {
      setTime(schedule[from][0]);
    }
    if (from === to) setTo("");
  }, [from]);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 20, backgroundColor: "#fff" }}>
      ...
      {showFromDropdown && (
        <View style={{ marginBottom: 10 }}>
          {regionalLocations.filter(l => l !== to).map(l => (
            <Card key={l} text={l} selected={from === l} onPress={() => { setFrom(l); setShowFromDropdown(false); }} />
          ))}
        </View>
      )}
      ...
      {showToDropdown && (
        <View style={{ marginBottom: 10 }}>
          {regionalLocations.filter(l => l !== from).map(l => (
            <Card key={l} text={l} selected={to === l} onPress={() => { setTo(l); setShowToDropdown(false); }} />
          ))}
        </View>
      )}
      ...
      <TouchableOpacity
        ...
        style={{ padding: 14, borderRadius: 12, borderWidth: 1, borderColor: "#ccc", marginBottom: 20 }}
      >
        ...
        <>
          ...
          {showTimeDropdown && (
            <View style={{ marginBottom: 20 }}>
              {schedule[from].map(t => (
                <Card key={t} text={t} selected={time === t} onPress={() => { setTime(t); setShowTimeDropdown(false); }} />
              ))}
            </View>
          )}
        </>
